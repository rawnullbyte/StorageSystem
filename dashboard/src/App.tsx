...
interface Bag {
  ...
  package_bill_no: string | null; container_display_name: string; layer_name: string; layer_id: number;
}
...
export default function App() {
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", fontFamily: "system-ui, sans-serif", fontSize: 14 }}>
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "6px 16px", borderBottom: "1px solid #ccc", background: "#f5f5f5" }}>
        <strong>StorageSystem</strong>
        <span style={{ fontSize: 12, color: "#666" }}>Inventory Manager</span>
        <span style={{ marginLeft: "auto", fontSize: 12, color: wsStatus === "connected" ? "#16a34a" : "#dc2626" }}>
          ● {wsStatus}
        </span>
      </div>
        ...
        <div style={{ width: 260, borderRight: "1px solid #ccc", overflow: "auto", background: "#fafafa", padding: 4, userSelect: "none" }}>
          ...
          <div style={{ display: "flex", gap: 2, margin: "6px 4px" }}>
            {(["layers","containers","bags"] as View[]).map(v => (
              <button key={v} onClick={() => setView(v)}
                style={{ flex: 1, padding: "4px 8px", border: "1px solid #ccc", borderRadius: 4, cursor: "pointer",
                  background: view === v ? "#e0e0e0" : "#fff", fontWeight: view === v ? 600 : 400, fontSize: 12 }}>{v}</button>
            ))}
          </div>
          ...
          {view === "containers" && (
            <div style={{ marginTop: 12, borderTop: "1px solid #ddd", paddingTop: 8 }}>
              <div onClick={() => setContainersExpanded(p => !p)} style={{ padding: "4px 8px", cursor: "pointer", fontWeight: 600, fontSize: 13, display: "flex", alignItems: "center", gap: 4 }}>
              ...
              {containersExpanded && containers.map(c => (
                <div key={c.id} onClick={() => setSelectedContainer(selectedContainer === c.id ? null : c.id)}
                  style={{ padding: "2px 8px 2px 24px", cursor: "pointer", fontSize: 12,
                    background: selectedContainer === c.id ? "#dbeafe" : "transparent" }}>
                  {c.display_name} <span style={{ color: "#999", fontSize: 10 }}>{c.id.slice(0,8)}…</span>
                </div>
              ))}
            </div>
          )}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "#f0f0f0", position: "sticky", top: 0 }}>
                  {view === "layers" && selectedContainer ? (
                    <><th style={th}>LCSC Part</th><th style={th}>MFG Part</th><th style={th}>Qty</th><th style={th}>Container</th><th style={th}>Layer</th></>
                  ) : view === "layers" && selectedLayer ? (
                    <><th style={th}>LCSC Part</th><th style={th}>MFG Part</th><th style={th}>Qty</th><th style={th}>Container</th></>
                  ) : view === "bags" ? (
                    <><th style={th}>LCSC Part</th><th style={th}>MFG Part</th><th style={th}>Qty</th><th style={th}>Container</th><th style={th}>Layer</th></>
                  ) : (
                    <><th style={th}>LCSC Part</th><th style={th}>MFG Part</th><th style={th}>Qty</th><th style={th}>Container</th><th style={th}>Layer</th></>
              ...
              <tbody>
                {filteredBags.length === 0 && (
                  <tr><td colSpan={5} style={{ padding: 24, textAlign: "center", color: "#999" }}>No components — scan a bag QR code</td></tr>
                )}
                {filteredBags.map((b, i) => (
                  <tr key={b.bag_id} style={{ background: i % 2 === 0 ? "#fff" : "#f9f9f9" }}>
                    <td style={td}>{b.lcsc_part_number}</td>
...
const th: React.CSSProperties = { padding: "6px 12px", textAlign: "left", fontSize: 12, fontWeight: 600, borderBottom: "2px solid #d0d0d0", whiteSpace: "nowrap" };
const td: React.CSSProperties = { padding: "4px 12px", borderBottom: "1px solid #e0e0e0", fontSize: 13, verticalAlign: "middle" };
